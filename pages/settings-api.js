document.addEventListener('DOMContentLoaded', () => {
    ...
    elements.tempRange.addEventListener('input', (e) => {
        ...
        // 3. 保存配置到 localStorage (使用全新的 vivre_api_settings 数据结构)
        elements.saveBtn.addEventListener('click', () => {
        
        // 确保 apiUrl 以 /chat/completions 结尾
        let finalApiUrl = elements.endpoint.value.trim() || 'https://api.openai.com/v1';
        if (!finalApiUrl.endsWith('/chat/completions')) {
            finalApiUrl = finalApiUrl.replace(/\/+$/, '') + '/chat/completions';
        }

        // 严格按照 vivre_api_settings 结构保存
        const apiConfig = {
            apiUrl: finalApiUrl,
            apiKey: elements.key.value.trim(),
            model: elements.modelSelect.value || 'gpt-3.5-turbo',
            stream: elements.stream.checked !== false, // 默认开启
            temperature: parseFloat(elements.tempRange.value || 1.0)
        };

        // 存入全新的 Key 中，供聊天室读取！
        localStorage.setItem('vivre_api_settings', JSON.stringify(apiConfig));
        
        // 为了兼容之前的旧逻辑（如果有的话），顺便也存一份旧的
        localStorage.setItem('ai_api_config', JSON.stringify(apiConfig));

        alert('API 设置已保存！现在可以去聊天室和 AI 对话了！');
    });
